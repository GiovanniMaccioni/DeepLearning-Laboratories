...
def evaluate_batch_adv(model, loader, criterion, class_target, iterations, epsilon, dataset, device, num_images):
    ...
    for (data, labels) in tqdm(loader, desc=f'Evaluating', leave=True):
        ...
        if num_images > 0:
            for i, adv in enumerate(adv_samples):
                ...
                if class_target == None:
                    ut.save_image(t, "./adv_samples/"+f"/a_{i}_non_targ_eps{epsilon}_.png", "")
                else:
                    ut.save_image(t, "./adv_samples/"+f"/b_{i}_targ_eps{epsilon}_ct{class_target}_it{iterations}.png", f"GT: {labels_names[labels[i]]} PRED:{labels_names[preds[i]]} ADV:{labels_names[preds_adv[i]]}")
    ...
    if class_target != None:
        ...
        ut.plot_confusion_matrix(labels_list, preds_adv_list, ut.get_labels(dataset), iterations, class_target, epsilon)
