...
def plot_ROC(logits_ID, logits_OOD, metric):
    ...
    #no skill prediction for majority class(In Distribution)
    ...
    plt.title(f"OOD Detetection on CIFAR 10 {metric}")
    plt.legend()
    #plt.show()
    plt.savefig(f'./plots/ROC_{metric}_noadv.png')
    plt.close()
    return
...
def plot_PrecisionRecall(logits_ID, logits_OOD, metric):
    ...
    plt.legend()
    #plt.show()
    plt.savefig(f'./plots/Precision_Recall_{metric}_noadv.png')
    plt.close()     
    return
...
def plot_histograms(logits_ID, logits_OOD, metric):
    ...
    plt.title(f"OOD Detetection on CIFAR 10 {metric}")
    #plt.show()
    plt.savefig(f'./plots/Histogram_{metric}_noadv.png')
    plt.close()
    return
...
def plot_confusion_matrix(y_true, y_pred, labels, iterations, class_target, epsilon):
    cm = confusion_matrix(y_true, y_pred)
    disp = ConfusionMatrixDisplay(confusion_matrix=cm,
                               display_labels=labels)
    disp.plot()
    #plt.show()
    plt.savefig(f'./plots/Confusion_matrix_eps{epsilon}_it{iterations}_ct{class_target}.png')
    plt.close()
    return
