    ...
    with torch.no_grad():
        for (data, labels) in tqdm(test_loader, desc=f'Activation Maps', leave=True):
            ...
            for i in range(data.shape[0]):
                ...
                save_image(data_cpu[i], f"./cam_results/{count}_orig_{label_list[labels[i]]}_p_{label_list[preds[i]]}_{count}.png", "Base")
                save_image(activation_maps[i].detach().cpu(), f"./cam_results/{count}_actv_map_{label_list[labels[i]]}_p_{label_list[preds[i]]}_{count}.png","Activation Map", colormap='jet')
                ...
                save_mix(data_cpu[i], activation_maps[i].detach().cpu(), f"./cam_results/{count}_mix_{label_list[labels[i]]}_p_{label_list[preds[i]]}_{count}.png", f"GT:{label_list[labels[i]]}  P:{label_list[preds[i]]}", colormap='jet')
                count +=1
